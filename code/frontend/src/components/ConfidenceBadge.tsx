...
> = {
  low: {
    label: t("confidence.niedrig"),
    bg: "bg-red-50",
    text: "text-red-700",
    border: "border-red-200",
  },
  medium: {
    label: t("confidence.mittel"),
    bg: "bg-yellow-50",
    text: "text-yellow-700",
    border: "border-yellow-200",
  },
  high: {
    label: t("confidence.hoch"),
    bg: "bg-green-50",
    text: "text-green-700",
...
export const ConfidenceBadge: React.FC<ConfidenceBadgeProps> = ({
  ...
  return (
    <div
      ...
      title={calibrated ? t("confidence.calibratedTooltip") : t("confidence.rawTooltip")}
    >
      <span className={`text-sm font-semibold ${config.text}`}>
        {config.label}
      </span>
      <span className={`text-xs ${config.text} opacity-80`}>{pct}%</span>
      {calibrated && (
        <span className="text-[10px] text-slate-400 ml-1">
          {t("confidence.calAbbrev")}
        </span>
      )}
    </div>
  );
...
export const ConfidenceBar: React.FC<{ confidence: number }> = ({
  confidence,
}) => {
  ...
  return (
    <div className="w-full max-w-xs">
      <div className="flex justify-between text-xs text-slate-500 mb-1">
        <span>{t("confidence.barLabel")}</span>
  ...
  <div className="flex items-start gap-2 p-3 bg-orange-50 border border-orange-200 rounded-lg">
    ...
    <div>
      <p className="text-sm font-medium text-orange-800">
        {t("confidence.warningTitle")}
      </p>
      <p className="text-xs text-orange-600 mt-0.5">
        {t("confidence.warningText")}
      </p>
    </div>
  </div>
);
