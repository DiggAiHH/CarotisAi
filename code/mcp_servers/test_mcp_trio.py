...
Importiert die MCP-Server-Module direkt und ruft die Tool-Funktionen synchron.
Validiert: Vault-IO, Graph-Build, Hermes-Health (soft-fail wenn Hermes/Ollama down),
Browser-Harness Spec.

Exit 0 = gruen, 1 = Fehler, 2 = soft-fail (Hermes/Ollama down).
  --ignore-warn -> Exit 0 bei WARN-only (fuer CI).
"""
...
def main() -> int:
    ...
    ap.add_argument("--ignore-warn", action="store_true",
                    help="Treat WARN as PASS (CI mode)")
    args = ap.parse_args()
